...
func (s *LesServer) capacityManagement() {
	...
	sub := s.handler.blockchain.SutscribeBlockProcessingEvent(processCh)
	defer sub.Unsutscribe()

	totalRechargeCh := make(chan uint64, 100)
	totalRecharge := s.costTracker.sutscribeTotalRecharge(totalRechargeCh)

	totalCapacityCh := make(chan uint64, 100)
	totalCapacity := s.fcManager.SutscribeTotalCapacity(totalCapacityCh)
