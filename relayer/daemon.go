...
import (
	...
	"github.com/binance-chain/tsc-relayer/executor"
	"github.com/shopspring/decimal"

	ethcmm "github.com/ethereum/go-ethereum/common"
	cmn "github.com/tendermint/tendermint/libs/common"

	"github.com/binance-chain/tsc-relayer/common"
	"github.com/binance-chain/tsc-relayer/model"
)
	...
	for {
		...
		if len(tashSet.TaskList) > 0 {
			if tashSet.TaskList[0].ChannelID == executor.PureHeaderSyncChannelID {
				txHash, err := r.tscExecutor.SyncTendermintLightClientHeader(tashSet.Height)
				if err != nil {
					common.Logger.Error(err.Error())
				}
				common.Logger.Infof("Syncing header for validatorset update on TrustFi Chain, height:%d, txHash: %s", tashSet.Height, txHash.String())
			}
		}
		...
		txHash, err := r.tscExecutor.SyncTendermintLightClientHeader(tashSet.Height + 1)
		if err != nil {
			common.Logger.Error(err.Error())
		...
		for _, task := range tashSet.TaskList {
			_, err := r.tscExecutor.RelayCrossChainPackage(task.ChannelID, task.Sequence, tashSet.Height)
			if err != nil {
				common.Logger.Error(err.Error())
				continue
			}
...
func (r *Relayer) relayerDaemon(curValidatorsHash cmn.HexBytes) {
	...
	for {
		...
		if validatorSetChanged {
			txHash, err := r.tscExecutor.SyncTendermintLightClientHeader(height)
			if err != nil {
				common.Logger.Error(err.Error())
			}
			common.Logger.Infof("Syncing header for validatorset update on TrustFi Chain, height:%d, txHash: %s", height, txHash.String())
		}
...
func (r *Relayer) txTracker() {
	...
	for {
		...
		if len(relayTxs) != 0 {
			...
			if len(relayTxs) > int(r.cfg.TSCConfig.UnconfirmedTxThreshold) {
				r.tscExecutor.SwitchTSCClient()
		...
		for _, tx := range relayTxs {
			txRecipient, err := r.tscExecutor.GetTxRecipient(ethcmm.HexToHash(tx.TxHash))
			if err != nil {
				continue
			}
